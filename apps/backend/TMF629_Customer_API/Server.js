    ...
    .then(async () => {
        ...
        if (count === 0) {
            await EventHub.create({
                callback: "http://localhost:4001/client/listener",
            });
            console.log(
                "📌 Default listener registered at http://localhost:4001/client/listener"
            );
        } else {
...
app.get("/", async (_req, res) => {
    try {
      ...
      <p>📡 Callback URL: <strong>http://localhost:4001/client/listener</strong></p>
